...
class DIPModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.net_input = self.get_noise(input_depth, 'noise', (self.opt.fineSize, self.opt.fineSize))
        ...
        self.optimizer_Net = torch.optim.Adam(
            itertools.chain(self.net_shared.parameters(), self.netDec_a.parameters()),
            lr=0.007, betas=(opt.beta1, 0.999)) # skip 0.01   # OST 0.001 # skip large 0.007
        self.optimizer_Dec_b = torch.optim.Adam(self.netDec_b.parameters(),
                                                lr=0.000007, betas=(opt.beta1, 0.999)) # OST 0.000007 skip 0.00002 skip large 0.000007
        self.optimizer_D_b = torch.optim.Adam(self.netD_b.parameters(), lr=0.0002, betas=(opt.beta1, 0.999))
    ...
    def set_input(self, inputs):
        ...
        input_A = inputs['A' if AtoB else 'B']
        input_B = inputs['B' if AtoB else 'A']
        if len(self.gpu_ids) > 0:
    ...
    def backward_G(self):
        # Train with A image - DIP loss
        fake_A = self.netDec_a(self.net_shared(self.net_input))
        loss_G_A = self.mse(fake_A, self.real_A)
    ...
    def get_current_errors(self):
        ret_errors = OrderedDict(
            [('D_b', self.loss_D_b),
             ('G_A', self.loss_G_A), ('G_B', self.loss_G_B) ])
        return ret_errors
