...
class ResNet(nn.Module):
    def __init__(self, num_input_channels, num_blocks, num_channels, need_residual=True, act_fun='LeakyReLU', need_sigmoid=True, norm_layer=nn.BatchNorm2d, pad='reflection'):
        '''
        ...
        layers = [
            ...
            act(act_fun),
            conv(num_channels, num_channels * 2, 3, stride=1, bias=True, pad=pad),
            act(act_fun),
            conv(num_channels * 2, num_channels * 4, 3, stride=1, bias=True, pad=pad),
            act(act_fun)
        ]
        # Residual blocks
        for i in range(num_blocks):
            layers += [s(*get_block(num_channels * 4, norm_layer, act_fun))]
       
        layers += [
            nn.Conv2d(num_channels * 4, num_channels* 2, 3, 1, 1),
            norm_layer(num_channels * 2, affine=True),
...
class ResNet_decoders(nn.Module):
    def __init__(self, num_channels, num_output_channels, pad='reflection'):
        super(ResNet_decoders, self).__init__()
        layers = [
